...
from rgbdsod.models.init_func import init_weight

from rgbdsod.models.logger import get_logger
...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        if cfg.backbone == 'swin_s':
            ...
            from rgbdsod.models.encoders.dual_swin import swin_s as backbone
            self.channels = [96, 192, 384, 768]
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'swin_b':
            logger.info('Using backbone: Swin-Transformer-Base')
            from rgbdsod.models.encoders.dual_swin import swin_b as backbone
            self.channels = [128, 256, 512, 1024]
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b5':
            logger.info('Using backbone: Segformer-B5')
            from rgbdsod.models.encoders.dual_segformer import mit_b5 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b4':
            logger.info('Using backbone: Segformer-B4')
            from rgbdsod.models.encoders.dual_segformer import mit_b4 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b2':
            logger.info('Using backbone: Segformer-B2')
            from rgbdsod.models.encoders.dual_segformer import mit_b2 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'mit_b1':
            logger.info('Using backbone: Segformer-B1')
            from rgbdsod.models.encoders.dual_segformer import mit_b0 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
            ...
            from rgbdsod.models import mit_b2 as backbone
            self.backbone = backbone(norm_fuse=norm_layer)
        elif cfg.backbone == 'sigma_tiny':  # use this
            logger.info('Using backbone: V-MAMBA')
            self.channels = [96, 192, 384, 768]
            from rgbdsod.models.encoders.dual_vmamba import vssm_tiny as backbone
            self.backbone = backbone()
        elif cfg.backbone == 'sigma_small':
            logger.info('Using backbone: V-MAMBA')
            self.channels = [96, 192, 384, 768]
            from rgbdsod.models.encoders.dual_vmamba import vssm_small as backbone
            self.backbone = backbone()
        elif cfg.backbone == 'sigma_base':
            logger.info('Using backbone: V-MAMBA')
            self.channels = [128, 256, 512, 1024]
            from rgbdsod.models.encoders.dual_vmamba import vssm_base as backbone
            self.backbone = backbone()
        else:
            logger.info('Using backbone: Segformer-B2')
            from rgbdsod.models.encoders.dual_segformer import mit_b2 as backbone
        ...
        if cfg.decoder == 'MLPDecoder':
            ...
            from rgbdsod.models.decoders.MLPDecoder import DecoderHead
            ...
            from rgbdsod.models.decoders.UPernet import UPerHead
            ...
            from rgbdsod.models.decoders.deeplabv3plus import DeepLabV3Plus as Head
        ...
        elif cfg.decoder == 'MambaDecoder':  # use this
            logger.info('Using Mamba Decoder')
            from rgbdsod.models.decoders.MambaDecoder import MambaDecoder
            ...
            from rgbdsod.models.decoders.fcnhead import FCNHead
    ...
    def encode_decode(self, rgb, modal_x):
        ...
        if not self.deep_supervision:
            ...
            out1 = F.interpolate(out[0], size=orisize[2:], mode='bilinear', align_corners=False)
            out2 = F.interpolate(out[1], size=orisize[2:], mode='bilinear', align_corners=False)
    ...
    def forward_0(self, rgb, modal_x, label=None):
        if not self.deep_supervision:
            if self.aux_head:
                out, aux_fm = self.encode_decode(rgb, modal_x)
            else:
...
if __name__=='__main__':
    ...
    model=EncoderDecoder(cfg=config).cuda()
    print(model)

    img1=torch.randn([1,3,224,224]).cuda()
    img2 = torch.randn([1, 3, 224, 224]).cuda()
