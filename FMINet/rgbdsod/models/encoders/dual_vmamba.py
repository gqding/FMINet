...
from rgbdsod.models.encoders.vmamba import Backbone_VSSM, CrossMambaFusionBlock, ConcatMambaFusionBlock
...
class RGBXTransformer(nn.Module):
    def __init__(self, 
                 ...
                 depths=[2, 2, 8, 2],  # [2,2,27,2] for vmamba tiny
                 dims=96,
                 pretrained=None,
                 mlp_ratio=4.0,
        ...
        self.cross_mamba = nn.ModuleList(  # this
        ...
        self.channel_attn_mamba = nn.ModuleList(  # this
    ...
    def forward_features(self, x_rgb, x_e):
        ...
        outs_rgb = self.vssm1(x_rgb) # B x C x H x W
        outs_x = self.vssm2(x_e) # B x C x H x W
...
class vssm_small(RGBXTransformer):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(vssm_small, self).__init__(
            depths=[2, 2, 27, 2],
            dims=96,
            pretrained='/gemini/pretrain/sunrgbd-sigma-small-epoch-320.pth',
            # pretrained='/media/ding/数据SSD/CodeBase/RGBD-SOD/SalMamba_freq/SalMamba_RGBD_SOD_384_pretrain_v1-1/rgbdsod/models/sunrgbd-sigma-small-epoch-320.pth',
            mlp_ratio=0.0,
            downsample_version='v1',
            drop_path_rate=0.3,
        )
