...
from rgbdsod.models.encoders.vmamba import CVSSDecoderBlock
...
class MambaDecoder(nn.Module):
    def __init__(self, 
        ...
        self.up_e = FinalUpsample_X4(input_resolution=(img_size[0] // patch_size, img_size[1] // patch_size),
                                      patch_size=4, dim=embed_dim)
        self.output_e = nn.Conv2d(in_channels=embed_dim, out_channels=self.num_classes, kernel_size=1, bias=False)
    ...
    def forward(self, inputs):
        if not self.deep_supervision:  # use this
            x = self.forward_up_features(inputs) # B, H, W, C
            x_last = self.up_x4(x, self.patch_size)
            x_last_e= self.up_x4_e(x, self.patch_size)
            return x_last, x_last_e
        else:
